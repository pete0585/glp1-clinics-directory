...
export default async function CityPage() {
  ...
  const faqLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "How many glp-1 weight loss clinic are in San Diego, CA?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "GLP-1 Weight Loss Clinic Directory lists 33+ glp-1 weight loss clinic in San Diego, California. Counts change as new listings are seeded.",
        },
      },
      {
        "@type": "Question",
        name: "How do I find glp-1 weight loss clinic in San Diego?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Search findglp1clinic.com and filter by San Diego. Compare listed providers, then contact the one that fits.",
        },
      },
    ],
  }
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <p className="text-sm text-neutral-500">San Diego, CA</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          GLP-1 Weight Loss Clinic in San Diego, CA
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          33+ listed glp-1 weight loss clinic in the San Diego area. Pages are generated from live directory listings — not outreach.
        </p>
          ...
          {listings.map((row: Record<string, unknown>, i: number) => (
            <li key={String(row.id || row.slug || i)} className="rounded-xl border border-neutral-200 p-4">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {String(row.city || "San Diego")}, {String(row.state || "CA")}
              </p>
            </li>
          ))}
        </ul>
