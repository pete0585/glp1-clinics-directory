...
export default function ListingDetail({ listing, monthlyViews }: ListingDetailProps) {
    const { id, clinic_name, address_line1, city, state, zip, phone, website, medications_offered, is_compounded, insurance_accepted, insurance_plans, physician_supervised, provider_credentials, monthly_price_min, monthly_price_max, clinic_type, telehealth_states, accepting_new_patients, listing_tier, claimed, } = listing;
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`card p-6 ${isFeatured ? 'border-amber-200' : ''}`}>
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              {accepting_new_patients && (<span className="badge badge-green flex-shrink-0">
                  <CheckCircle className="h-3 w-3"/>
                  Accepting Patients
                </span>)}
            </div>

            {fullAddress && (<div className="flex items-start gap-2 text-sm text-gray-600 mb-3">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              {(<>
                  {phone && (<a href={`tel:${phone}`} className="flex items-center gap-1.5 text-teal hover:text-teal-600 font-medium">
                      <Phone className="h-4 w-4"/>
                      {phone}
                    </a>)}
                  {website && (<a href={website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-teal hover:text-teal-600 font-medium">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {clinic_type && (<div>
                  ...
                  {telehealth_states && telehealth_states.length > 0 && (<p className="text-xs text-gray-500 mt-1">
            ...
            : 'Insurance not accepted'}
                ...
                {insurance_plans && insurance_plans.length > 0 && (<p className="text-xs text-gray-500 mt-1">{insurance_plans.join(', ')}</p>)}
              </div>

              {physician_supervised !== undefined && (<div>
          ...
          {!isClaimed && (<div className="rounded-2xl bg-teal-50 border border-teal-100 p-5 text-sm">
              <p className="font-semibold text-teal-700 mb-1">Is this your clinic?</p>
              <p className="text-teal-600 mb-3"> Listings are free. Public contact details are available without payment. </p>
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5 space-y-3">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              {zip && (<div className="flex items-center justify-between">
              ...
              {clinic_type && (<div className="flex items-center justify-between">
                  <span className="text-gray-500">Visit Type</span>
                  <span className="font-medium">{getClinicTypeLabel(clinic_type)}</span>
                </div>)}
              {listing_tier !== 'free' && (<div className="flex items-center justify-between">
