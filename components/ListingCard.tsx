...
export default function ListingCard({ listing, featured }: ListingCardProps) {
    const { clinic_name, slug, city, state, phone, website, medications_offered, is_compounded, insurance_accepted, physician_supervised, provider_credentials, monthly_price_min, monthly_price_max, clinic_type, listing_tier, claimed, } = listing;
    ...
    return (<Link href={`/listings/${slug}`} className="block">
      <div className={`card p-5 group h-full flex flex-col ${isFeatured ? 'border-amber-200 bg-amber-50/30 shadow-card' : ''}`}>
        ...
        {/* Medication badges */}
        {medLabels.length > 0 && (<div className="flex flex-wrap gap-1.5 mb-3">
            {medLabels.slice(0, 3).map((med) => (<span key={med} className="badge badge-teal">{med}</span>))}
            {medLabels.length > 3 && (<span className="badge badge-gray">+{medLabels.length - 3} more</span>)}
          </div>)}

        {/* Badges row */}
        <div className="flex flex-wrap gap-2 mb-3">
          {physician_supervised && (<span className="badge badge-green flex items-center gap-1">
              <Stethoscope className="h-3 w-3"/>
              {provider_credentials ? `${provider_credentials}-Supervised` : 'Physician-Supervised'}
            </span>)}
          {insurance_accepted && (<span className="badge badge-teal flex items-center gap-1">
              <ShieldCheck className="h-3 w-3"/>
              Insurance
            </span>)}
          {clinic_type === 'telehealth' || clinic_type === 'hybrid' ? (<span className="badge badge-gray flex items-center gap-1">
        ...
        <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 text-xs text-gray-500">
            {price && (<span className="flex items-center gap-1 font-semibold text-teal">
          ...
          {website && (<span className="flex items-center gap-1 text-xs text-teal-400">
